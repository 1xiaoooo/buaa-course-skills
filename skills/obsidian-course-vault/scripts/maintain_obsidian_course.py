...
def build_semantic_rebuild_prompt(mode: str) -> str:
    ...
    concept_admission_rules = [
        "- 写正文前先确定本节正式概念集：只能包含已有概念页，或你决定本轮同步写成概念页并接入图谱的新概念。",
        "- 没有页面且本轮不准备落页的术语，只能作为普通文本出现，不能写进 `concepts` frontmatter，也不能写成 wiki link。",
        "- 拒绝弱概念提示：章节标题、泛泛名词、一次性例子、PPT/OCR-only 术语、ASR 噪声、以及无法从完整转写和课程上下文解释的词。",
    ]
    return "\n".join(
        [
            ...
            "- `内容纪要` 的每个分段都必须保留时间轴。时间轴必须写成可直接定位回放的时间戳，例如“时间参考：约 `03:29-18:58`”或跨小时的“时间参考：约 `49:40-01:05:29`”。超过 1 小时必须写三段式 `HH:MM:SS`，不要把第 80 分钟写成 `01:20`。",
            *concept_admission_rules,
            "- 已有概念页如果属于本节正式概念集，必须在 `主题`、`本节主线 / 内容纪要`、`本节提到的概念` 中作为 wiki link 出现。",
            ...
            "- 对概念升格和 hub 增长先做准入判断；只有 transcript-stable 且能同步落成正式概念页的概念，才能进入正式 lesson 概念集。",
        ]
    )
